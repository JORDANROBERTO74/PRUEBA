...
export function ChatPanel() {
  ...
  if (messages.length > 0 && !isButtonPressed) {
    return (
      <div className="w-full flex justify-center items-center">
        <div className="fixed bottom-16 sm:bottom-16 md:bottom-8 lg:bottom-8 z-[10]">
          <Button
            type="button"
            className="rounded-full group transition-all hover:scale-105"
            onClick={() => handleClear()}
          >
  ...
  const formPositionClass =
    messages.length === 0
      ? 'bottom-8 left-0 right-0 top-10 mx-auto h-full flex flex-col items-center justify-center'
      : 'bottom-8-ml-6'
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl w-full px-6">
        <div className="relative flex items-center w-full">
          <Input
            ...
            onBlur={() => setShowEmptyScreen(isExampleClicked ? true : false)}
          />
          <Button
            type="submit"
            size={'icon'}
        ...
        <EmptyScreen
          ...
          setIsExampleClicked={setIsExampleClicked}
        />
      </form>
    </div>
  )
